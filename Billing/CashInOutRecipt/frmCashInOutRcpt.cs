...
namespace POS.Billing.CashInOutRecipt
{
    public partial class frmCashInOutRcpt : Form
    {
        Billing.frmPosCashInOut fpc = null;
        public frmCashInOutRcpt(Billing.frmPosCashInOut _fpc)
        {
            InitializeComponent();
            fpc = _fpc;
        }

        private void frmCashInOutRcpt_Load(object sender, EventArgs e)
        {                                   
            cashInOutParam();
            
        }
        private void cashInOutParam()
        {
            ...
            ReportParameter date = new ReportParameter("date", DateTime.Now.ToString("MM/dd/yyy " + "(ddd)"));
            ReportParameter hr = new ReportParameter("hour", DateTime.Now.ToString("hh:mm tt"));

            ReportParameter cashIO = new ReportParameter("cashInOut", "Cash " + fpc.cmbInOut.Text);
            ReportParameter cashAmt = new ReportParameter("cashInOutAmt", fpc.txtAmt.Text);
            ReportParameter mem = new ReportParameter("memo", fpc.txtMemo.Text);
            reportViewer1.LocalReport.SetParameters(new ReportParameter[] { prop, company, tin, min, add, posDesc, serialNo, addedBy,date, hr ,cashIO,cashAmt,mem});
            reportViewer1.RefreshReport();
        }
        public void autoPrint_CashInOutReceipt()
        {
            cashInOutParam();
        ...
        private void frmCashInOutRcpt_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Dispose();
        }
    }
